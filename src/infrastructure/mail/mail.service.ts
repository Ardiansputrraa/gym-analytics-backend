...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset Your Password - Gym Analytics</title>
</head>
<body style="margin: 0; padding: 0; background-color: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #f8fafc;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #0f172a; padding: 40px 20px;">
    <tr>
        ...
        <table role="presentation" width="100%" max-width="520px" cellspacing="0" cellpadding="0" style="max-width: 520px; background-color: #1e293b; border-radius: 16px; border: 1px solid #334155; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <p style="margin: 0 0 24px 0; font-size: 14px; line-height: 1.6; color: #cbd5e1;">
                We received a request to reset your password. Use the following 6-digit OTP code to set a new password for your account:
              </p>
              <!-- OTP Box -->
              <div style="background-color: #0f172a; border-radius: 12px; border: 1px solid #e11d48; padding: 20px; text-align: center; margin-bottom: 24px;">
                <span style="font-family: 'Courier New', Courier, monospace; font-size: 36px; font-weight: 800; letter-spacing: 8px; color: #fb7185;">
                  ${otp}
                </span>
              </div>
              <p style="margin: 0 0 8px 0; font-size: 13px; color: #94a3b8; text-align: center;">
    ...
    const textContent = `Your Gym Analytics password reset code is: ${otp}\n\nThis code will expire in ${expiresInMinutes} minutes.\n\nIf you did not request a password reset, please ignore this email.`;

    try {
      await this.transporter.sendMail({
        from,
        to,
        subject: `${otp} is your Gym Analytics password reset code`,
        text: textContent,
        html: htmlContent,
      });
